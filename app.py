...
dispatcher.connect(name='api_create_game', route='/game/create', controller=api.controllers.CreateGameController(), action='index', conditions=dict(method=['GET']))
dispatcher.mapper.connect('/game/create', controller='api_create_game', action='create', conditions=dict(method=['POST']))

dispatcher.connect(name='api_get_game', route='/game/{game_id}/player/{player_id}', controller=api.controllers.GameController(), action='index', conditions=dict(method=['GET']))

dispatcher.connect(name='api_join_game', route='/game/join', controller=api.controllers.JoinGameController(), action='index', conditions=dict(method=['GET']))
dispatcher.mapper.connect('/game/join', controller='api_join_game', action='join', conditions=dict(method=['POST']))

dispatcher.connect(name='api_start_game', route='/game/start', controller=api.controllers.StartGameController(), action='index', conditions=dict(method=['GET']))
dispatcher.mapper.connect('/game/start', controller='api_start_game', action='start', conditions=dict(method=['POST']))

dispatcher.connect(name='api_draw_cards', route='/game/draw', controller=api.controllers.DrawCardsController(), action='index', conditions=dict(method=['GET']))
dispatcher.mapper.connect('/game/draw', controller='api_draw_cards', action='draw', conditions=dict(method=['POST']))

dispatcher.connect(name='api_discard_cards', route='/game/discard', controller=api.controllers.DiscardCardsController(), action='discard', conditions=dict(method=['GET']))
...
#conf['/static'] = {'tools.staticdir.on': True, 'tools.staticdir.dir': "public"}
